...
class ModelExtensionShippingMatkahuolto extends Model {
	function getQuote($address) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('matkahuolto_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if (!$this->config->get('matkahuolto_geo_zone_id')) {
			$status = true;
		...
		foreach($products as $product){
            ...
            $longest = $this->get_longest($product['length'], $product['width'], $product['height'], $product['length_class_id']);
		        if($volume > '0.5'){
                   ...
                   if(!$this->config->get('matkahuolto_iso_palvelu')){
			      	    $status = false;
                        $error = $this->language->get('error_volume');
                   }
                   if($this->config->get('matkahuolto_iso_palvelu') && $volume >= '0.82'){
                        $status = false;
                        $error = $this->language->get('error_volume');
                   }
		    	}
                ...
                if($longest >= '1.2' && $longest < 3){
                  ...
                  if(!$this->config->get('matkahuolto_iso_palvelu') && !$error){ 
		        		$status = false;
                        $error3 = $this->language->get('error_volume');
                  }
                  if($this->config->get('matkahuolto_iso_palvelu') && $this->config->get('matkahuolto_iso_price')){
               	      $add = $this->config->get('matkahuolto_iso_price');
                      $list = 'M';
                  }
               }
        ...
        $iso_codes = array('FI', 'BE', 'DK', 'EE', 'DE', 'AT', 'NL', 'LT', 'LV', 'LU', 'SE');
         ...
         if($address['iso_code_2'] == 'FI'){
            ...
            if($this->config->get('matkahuolto_kotijakelu')){ 
               $kotijakelu = true;
            }
            
         }
         if($address['iso_code_2'] == 'SE' || $address['iso_code_2'] == 'DK'){
             if($this->config->get('matkahuolto_hinnasto_2_status')){
         ...
         if($address['iso_code_2'] == 'EE' || $address['iso_code_2'] == 'LT' || $address['iso_code_2'] == 'LV'){
            if($this->config->get('matkahuolto_hinnasto_3_status')){
               // Ruotsi, Tanska ja Baltian maat
         ...
         if($address['iso_code_2'] == 'DE' || $address['iso_code_2'] == 'AT' || $address['iso_code_2'] == 'BE' || $address['iso_code_2'] == 'LU' || $address['iso_code_2'] == 'NL'){
            if($this->config->get('matkahuolto_hinnasto_4_status')){
   ...
   if ($status && in_array($address['iso_code_2'], $iso_codes)) {
          if($this->config->get('matkahuolto_test_mode')){
        ...
        $array = simplexml_load_string($response);
         ...
         $ale = '';
         $koti = $this->config->get('matkahuolto_kotijakelu_price');
         $rahtivapaa = '';

         if($total  >= $this->config->get('matkahuolto_cargo_sum')){
              if($this->config->get('matkahuolto_free_cargo')){
                  $cost = 0;
                  $koti = 0;
              } elseif ($this->config->get('matkahuolto_discount_cargo') && $this->config->get('matkahuolto_discount_cargo_percent')){
                  $summa = ($cost/100)*$this->config->get('matkahuolto_discount_cargo_percent');
                  $cost = round($cost-$summa,2);
                  $koti = $this->config->get('matkahuolto_kotijakelu_price');
                  $ale = '(Alennus ' . $this->config->get('matkahuolto_discount_cargo_percent') . '%)';
              }
         }
           ...
           foreach($array->Office as $place){
               ...
               $quote_data[(int)$place->SeqNumber] = array(
                   ...
                   'cost'         => trim($cost+$add),
                   'tax_class_id' => $this->config->get('matkahuolto_tax_class_id'),
                   'text'         => $this->currency->format($this->tax->calculate(trim($cost+$add), $this->config->get('matkahuolto_tax_class_id')), $this->session->data['currency'])
                    );
               }
               ...
               $quote_data['kotijakelu'] = array(
                    ...
                    'cost'         => $cost + $koti,
                    'tax_class_id' => $this->config->get('matkahuolto_tax_class_id'),
                    'text'         => $this->currency->format($this->tax->calculate($cost + $koti, $this->config->get('matkahuolto_tax_class_id')), $this->session->data['currency'])
                    );
              }
          }
	   ...
	   $method_data = array();
		...
		if ($quote_data) {
			...
			$method_data = array(
				...
				'title'      => $title,
				'quote'      => $quote_data,
				'sort_order' => $this->config->get('matkahuolto_sort_order'),
				'error'      => false
			);
		}
		return $method_data;
	...
	protected function cost($weight){
		foreach($this->config->get('matkahuolto_hinnasto') as $result){
			if($weight <= $result['kg']){
				return $result['price'];
			}
		}
	...
	protected function cost2($weight){
		foreach($this->config->get('matkahuolto_hinnasto_2') as $result){
			if($weight <= $result['kg']){
				return $result['price'];
			}
		}
	...
	protected function cost3($weight){
		foreach($this->config->get('matkahuolto_hinnasto_3') as $result){
			if( $weight <= $result['kg']){
				return $result['price'];
			}
		}
	...
	protected function cost4($weight){
		foreach($this->config->get('matkahuolto_hinnasto_4') as $result){
			if($weight <= $result['kg']){
				return $result['price'];
			}
		}
    ...
    public function sendXML($address, $list){
        if($this->config->get('matkahuolto_search_result')){
           $result = $this->config->get('matkahuolto_search_result');
        } else {
           $result = 5;
        }
        ...
        if($this->config->get('matkahuolto_test_mode')){
           ...
           $user_id = (int)$this->config->get('matkahuolto_user_id');
        }
